...
function densityGroups(data: DataView, config: ChartConfig): DensityGroup[] {
  ...
  if (!(groupCol in data.columnArrays)) {
    return [];
  }
  const groupData = data.columnArrays[groupCol] as unknown[];
...
function ridgeSeries(groups: DensityGroup[], theme: ThemeTokens): EChartsOption['series'] {
...
class RidgelinePlotRenderer extends EChartsBaseRenderer {
  ...
  buildOption(data: DataView, config: ChartConfig, theme: ThemeTokens): EChartsOption {
    ...
    return {
      ...
      series: ridgeSeries(groups, theme),
...
chartRegistry.register({
  ...
  family: 'distribution',
  name: 'Ridgeline Plot',
  description: 'Stacked kernel-density curves for grouped numeric distributions',
  renderer: 'echarts',
  ...
  createRenderer: () => new RidgelinePlotRenderer(),
});
