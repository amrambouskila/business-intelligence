...
function percentMatrix(keys: string[], groups: string[], matrix: number[][]): number[][] {
...
class PercentStackedAreaRenderer extends EChartsBaseRenderer {
  ...
  buildOption(data: DataView, config: ChartConfig, theme: ThemeTokens): EChartsOption {
    ...
    const percentages = percentMatrix(keys, groups, matrix);
    const axes = buildCartesianAxes(
      theme,
      { type: 'category', data: keys, splitLine: false },
...
chartRegistry.register({
  ...
  description: 'Long-form time series normalized to 100% at each time bucket',
  renderer: 'echarts',
  ...
  createRenderer: () => new PercentStackedAreaRenderer(),
});
