...
const optionSpecs: ChartOptionSpec[] = [
  { key: 'opacity', label: 'Opacity', control: 'number', default: 0.56, min: 0.1, max: 1, step: 0.05 },
  { key: 'lineWidth', label: 'Line width', control: 'number', default: 2, min: 0, max: 8, step: 1 },
];

class FilledMapRenderer extends DeckGLBaseRenderer {
...
chartRegistry.register({
  ...
  family: 'geographic',
  name: 'Filled Map',
  description: 'Filled geographic regions styled by value',
  renderer: 'deckgl',
  ...
  options: optionSpecs,
  createRenderer: () => new FilledMapRenderer(),
});
