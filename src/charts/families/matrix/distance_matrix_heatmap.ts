...
class DistanceMatrixHeatmapRenderer extends EChartsBaseRenderer {
  protected isEmpty(data: DataView, config: ChartConfig): boolean {
    return matrixGrid(data, config).finiteValues.length === 0;
  }

  protected emptyMessage(): string {
    return 'No distance values to chart';
  }
...
chartRegistry.register({
  type: 'distance_matrix_heatmap',
  family: 'matrix',
  name: 'Distance Matrix Heatmap',
  description: 'Pairwise distance matrix rendered as a sequential heatmap',
  renderer: 'echarts',
  ...
  requiredColumns: [
    ...
    { role: 'value', acceptedTypes: ['numeric', 'integer', 'float'], label: 'Distance' },
  ],
  createRenderer: () => new DistanceMatrixHeatmapRenderer(),
});
