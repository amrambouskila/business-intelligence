...
function confusionGrid(data: DataView, config: ChartConfig) {
...
class ConfusionMatrixRenderer extends EChartsBaseRenderer {
  protected isEmpty(data: DataView, config: ChartConfig): boolean {
    return confusionGrid(data, config).finiteValues.length === 0;
  }

  protected emptyMessage(): string {
    return 'No confusion-matrix values to chart';
  }

  buildOption(data: DataView, config: ChartConfig, theme: ThemeTokens): EChartsOption {
    const gridData = confusionGrid(data, config);
...
chartRegistry.register({
  ...
  description: 'Actual versus predicted class counts',
  renderer: 'echarts',
  ...
  createRenderer: () => new ConfusionMatrixRenderer(),
});
