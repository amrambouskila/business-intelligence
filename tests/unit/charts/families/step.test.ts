...
import '@/charts/families/time-series/step';
...
describe('step registration', () => {
  it('registers under type "step" with the time-series family', () => {
    const def = chartRegistry.get('step');
...
describe('step buildOption', () => {
  const def = () => chartRegistry.get('step')!;

  it('uses a stepped line series with category axis when x is not datetime', () => {
    const dv: DataView = {
    ...
    const cfg: ChartConfig = { chartType: 'step', columns: { x: 'cat', y: 'y' }, options: {} };
    ...
    const series = opt.series as Array<{ type: string; step: string; data: unknown[] }>;
  ...
  it('uses time axis and paired [x, y] data when x is datetime', () => {
    ...
    const cfg: ChartConfig = { chartType: 'step', columns: { x: 'ts', y: 'y' }, options: {} };
    const opt = (def().createRenderer() as EChartsBaseRenderer).buildOption(dv, cfg, theme()) as EChartsOption;
    expect((opt.xAxis as Record<string, unknown>).type).toBe('time');
    const series = opt.series as Array<{ step: string; data: unknown[] }>;
    expect(series[0].step).toBe('end');
  ...
  it('falls back to empty column arrays if references are missing', () => {
    ...
    const cfg: ChartConfig = { chartType: 'step', columns: { x: 'missing', y: 'also_missing' }, options: {} };
    const opt = (def().createRenderer() as EChartsBaseRenderer).buildOption(dv, cfg, theme()) as EChartsOption;
