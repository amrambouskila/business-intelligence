...
describe('line registration', () => {
  it('registers under type "line" with the time-series family', () => {
    const def = chartRegistry.get('line');
...
describe('line buildOption', () => {
  const def = () => chartRegistry.get('line')!;

  it('uses category axis when x is not datetime', () => {
    const dv: DataView = {
  ...
  it('uses time axis and paired [x, y] data when x is datetime', () => {
    ...
    const cfg: ChartConfig = { chartType: 'line', columns: { x: 'ts', y: 'y' }, options: {} };
    const opt = (def().createRenderer() as EChartsBaseRenderer).buildOption(dv, cfg, theme()) as EChartsOption;
  ...
  it('falls back to empty column arrays if references are missing', () => {
    ...
    const cfg: ChartConfig = { chartType: 'line', columns: { x: 'missing', y: 'also_missing' }, options: {} };
    const opt = (def().createRenderer() as EChartsBaseRenderer).buildOption(dv, cfg, theme()) as EChartsOption;
    const series = opt.series as Array<{ data: unknown[] }>;
