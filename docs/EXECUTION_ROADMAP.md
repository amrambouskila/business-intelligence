...
- **U1 — mostly resolved for navigation/basic compositing** — the Layers tab can activate layers and toggle each layer's `visible` state, and `ChartArea` mounts all visible, fillable layers into one render root for basic overlay composition. Shared-axis option merging and `axis: y1/y2` semantics remain future Phase 3 work.
...
Scope:
- **deck.gl vertical slice (own gated sub-milestone):** **view abstraction (Map/Orbit/Orthographic), data-driven `getInitialViewState` from data bounds, all fourteen geographic charts, and all six 3D charts shipped in M5 slices 2–6**; repeated real-GL chart switching is now covered by `tests/e2e/deckgl-switch-stress.spec.ts`. *(C3 already re-examined as a false positive in M0)* *(fixes A5)*
