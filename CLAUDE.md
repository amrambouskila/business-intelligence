...
| Git remote | GitHub (public) — CI via GitHub Actions per global CLAUDE.md §5 |
...
## 3. Stack Overrides from Global CLAUDE.md
...
- 4 backends, each chart picks one:
  ...
  - **DeckGLBaseRenderer** — subclasses override `buildLayers()`; cleanup handled by the `DeckGLChart` wrapper FC which calls `deck.finalize()` on unmount
  - **ReglRenderer** — custom WebGL via regl (not yet implemented)
  - **Canvas2DRenderer** — fallback for simple charts (not yet implemented)
...
Each store is in its own file (CLAUDE.md §7 "one concept per file"). Selectors that derive across stores (`useActiveDataset`, `useActiveChartConfig`) live alongside their primary store.
...
- **Coverage target: 100%** per global CLAUDE.md §7 — enforced in `vitest.config.ts` thresholds; CI fails on drop.
...
- **Provider wiring (public GitHub repo, wired):** `github/codeql-action` (init → analyze, language `javascript-typescript`) **plus** `pipx run semgrep scan` uploading SARIF via `github/codeql-action/upload-sarif` (a following step fails the job when Semgrep reported findings), so findings land in Security → Code scanning. `gitleaks/gitleaks-action@v2` for secrets. `aquasecurity/trivy-action@v0.36.0` (`severity: HIGH,CRITICAL`, `exit-code: 1`) against the image built in the `docker` job. Job-level `permissions: { security-events: write, contents: read }`.
...
4. **CLAUDE.md / master plan** — update if the change touches a contract, a rule, or a phase-level scope.
